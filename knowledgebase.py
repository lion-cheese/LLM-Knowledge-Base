...
TARGET_SOURCE_CHUNKS = 4    # Number of relevant chunks to return when retrieving
CHUNK_SIZE = 500            # Number of characters per chunk
CHUNK_OVERLAP = 50          # Overlap between chujnks to preserve context
HIDE_SOURCE_DOCUMENTS = False

class PDFKnowledgeBase:
    # Initialize the PDFKnowledgeBase instance
    def __init__(self, pdf_course_folder_path: str, chroma_settings: dict) -> None:
        self.pdf_course_folder_path = pdf_course_folder_path
    
    # Load PDF documents from the specified directory
    def load_pdfs(self):
        loader = DirectoryLoader(self.pdf_cours_folder_path)
    ...
    # Convert document chunks to embeddings and store them in the Chroma vector database
    def convert_document_to_embeddings(self, chunked_docs, embedder):
        vector_db = Chroma(persist_directory=CHROMA_DB_DIRECTORY, embedding_function=embedder, client_settings=CHROMA_SETTINGS)
        vector_db.add_documents(chunked_docs)
        vector_db.persist()
        return vector_db
    
    # Retrieve a retriever object from the persistent vector database
